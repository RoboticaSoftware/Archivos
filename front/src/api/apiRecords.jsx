...
export async function getRecordsTypeApi () {
    /**
     * Obtiene los tipos de solicitudes que se pueden hacer, en las PQRSD
     */
    const url = `${BASE_API}/api/recordtype/`
    const params = {
        method: "GET"
    };
...
export async function createRecordsApi (formData, setPQRSD){
    /**
     * Esta función me permite enviar una solicitud http para creaer un radicado 
     */
    if ('r_number' in formData) {
        delete formData['r_number'];
    }

    const url = `${BASE_API}/api/records/`
    const params = {
        method: "POST",
        headers: {
